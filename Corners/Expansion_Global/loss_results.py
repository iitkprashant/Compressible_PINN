...
# # plt.plot(data["Epoch"], data["Pressure"], label='Pressure')
...
for i, var in enumerate(variables):
    axes[i].plot(data["Epoch"][:200], data[var][:200], label=titles[i], color=colors[i])
...
# fig.suptitle('Variables Over Iterations', fontsize=16, weight='bold', y=1.02)
...
fig.supxlabel('Epochs', fontsize=18)
...
# #plt.plot(data["Epoch"], data["Sigma1"], label='s1')
# #plt.plot(data["Epoch"], data["Sigma2"], label='s2')
# plt.yscale('log')
...
plt.savefig('nu.png', dpi=300, bbox_inches='tight')
plt.show()

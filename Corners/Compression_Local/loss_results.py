...
bc_variables = ["Inlet", "base", "Ramp", "Top"]
bc_titles = ['Inlet', 'Base', 'Ramp', 'Top']
...
for i, var in enumerate(pde_variables):
    axes[i].plot(data["Epoch"], data[var], label=pde_titles[i], color=pde_colors[i])
    axes[i].set_yscale('log')
...
# Plot BC variables
fig, axes = plt.subplots(nrows=1, ncols=len(bc_variables), figsize=(16, 4), constrained_layout=True)

for i, var in enumerate(bc_variables):
    axes[i].plot(data["Epoch"], data[var], label=bc_titles[i], color=bc_colors[i])
    axes[i].set_yscale('log')

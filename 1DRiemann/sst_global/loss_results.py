...
plt.plot(data["Epoch"][:300], data["mu"][:300], label=r'$\nu_{1}$ (Exp 1)', 
         linestyle='-', marker='o', markersize=2, linewidth=1.5, color='blue')
plt.plot(data1["Epoch"][:300], data1["mu"][:300], label=r'$\nu_{2}$ (Exp 2)', 
...
pde_variables = [
    ("Continuity", data["Cont"][:300], 'firebrick', '-', 'o'),
    (r"Momentum ($x$)", data["Mom_x"][:300], 'firebrick', '--', 's'),
    ("Energy", data["Energy"][:300], 'firebrick', '-.', '^'),
]

# Plot each PDE loss variable
for i, (label, values, color, linestyle, marker) in enumerate(pde_variables):
    axs[i].plot(data["Epoch"][:300], values, label=label, 
                linestyle=linestyle, marker=marker, markersize=2, linewidth=1.5, color=color)
    axs[i].set_yscale('log')
    axs[i].set_xlabel('Epochs', fontsize=18)
    axs[i].set_ylabel('Loss', fontsize=18)
...
ic_variables = [
    ("Initial Condition (Left)", data["Initialleft"][:300], 'firebrick', '-', 'D'),
    ("Initial Condition (Right)", data["Initialright"][:300], 'firebrick', '--', 'x'),
]

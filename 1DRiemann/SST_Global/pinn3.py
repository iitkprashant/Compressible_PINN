...
class PINN(nn.Module):
    ...
    def _physics_loss(self, x_train):
        ...
        self.mu = self.model2(x_train)**2  # Dynamic viscosity
        ...
        r1 = torch.mean(self.lam * res1**2)

        res2 = (U2_t + f2_x - self.mu * U2_xx)
        r2 = torch.mean(self.lam * res2**2)

        res3 = (U3_t + f3_x - self.mu * U3_xx)
        r3 = torch.mean(self.lam * res3**2)
    ...
    def save_predictions1(self, grid, save_path):
        self.model.eval()
        with torch.no_grad():
            predictions = self.model(grid).cpu().numpy()
    ...
    def train(self, x_physics, x_boundary, u_boundary,epochs=501):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 1000==0:
                ...
                y = torch.arange(0, 0.2+step_size_time, step_size_time)
                ...
                model.save_predictions(grid, './predict/predictions11.npy')
                model2.save_predictions(grid, './predict/predictions21.npy')
    ...
    def train_lbfgs(self, x_physics, x_boundary, u_boundary, epochs=2001):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                ...
                model.save_predictions(grid, f'./predict/predictions_lbfgs21.npy')
                model2.save_predictions(grid, f'./predict/predictions2_lbfgs21.npy')
...
if __name__ == "__main__":
    ...
    interior_points = polygon.generate_random_points_inside(30000)
    ...
    pinn.train(x_physics=inputs, x_boundary=x_boundary, u_boundary=u_initial, epochs=501)

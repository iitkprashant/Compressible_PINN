...
class PINN(nn.Module):
    def __init__(self, model, device='cpu'):
        ...
        self.model = model
        self.device = device
        self.raw_mu = torch.nn.Parameter(torch.tensor([-4.0], requires_grad=True, device=self.device, dtype=torch.float32))
    ...
    def train(self, x_physics, x_boundary, u_boundary,epochs=501, batch_size=8192):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 1000==0:
                ...
                y = torch.arange(0, 0.02+step_size_time, step_size_time)

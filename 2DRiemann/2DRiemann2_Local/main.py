...
class PINN(nn.Module):
    ...
    def _initial_loss(self, x_initial, ic):
        ...
        tr_ic = torch.cat([
            ic[0,0]*torch.ones((top_right.shape[0], 1), device=self.device),          
            ic[0,3]*torch.ones((top_right.shape[0], 1), device=self.device),          
            ic[0,1]*  torch.ones((top_right.shape[0], 1), device=self.device),    
            ic[0,2]* torch.ones((top_right.shape[0], 1), device=self.device)     
        ], dim=1).to(self.device)

        bl_ic = torch.cat([
            ic[2,0]*torch.ones((bottom_left.shape[0], 1), device=self.device),       
            ic[2,3]*torch.ones((bottom_left.shape[0], 1), device=self.device),           
            ic[2,1]*  torch.ones((bottom_left.shape[0], 1), device=self.device),    
            ic[2,2]* torch.ones((bottom_left.shape[0], 1), device=self.device)      
        ], dim=1).to(self.device)
        
        br_ic = torch.cat([
            ic[3,0]*torch.ones((bottom_right.shape[0], 1), device=self.device),          
            ic[3,3]*torch.ones((bottom_right.shape[0], 1), device=self.device),          
            ic[3,1]*  torch.ones((bottom_right.shape[0], 1), device=self.device),    
            ic[3,2]* torch.ones((bottom_right.shape[0], 1), device=self.device)     
        ], dim=1).to(self.device)

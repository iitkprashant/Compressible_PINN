...
mu = 0.01 * AVdata[:,2].reshape(101,101, 41)**2 
...
labels = ['U-velocity', 'V-velocity', 'Pressure', 'Speed', 'Density', 'AV' ]

for field_name, data, filename, label in zip(fields.keys(), fields.values(), filenames, labels):
    plt.figure(figsize=(8, 6))
    contour = plt.contourf(x[:,:,-1], y[:,:,-1], data, cmap='jet', levels=100, extend='both')
